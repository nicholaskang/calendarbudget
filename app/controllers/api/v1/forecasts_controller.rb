class Api::V1::ForecastsController < ApplicationController
  ...
  def update
   ...
   goals = Goal.update(goal_type_id: params[:goal_type_id], total_amount: params[:total_amount], contribution: params[:contribution], frequency_id: params[:frequency_id], goal_start_date: params[:goal_start_date], goal_end_date: params[:goal_end_date])   
...
# ****************************************************************************************************************************************************************
  ...
  def all_income_hashes_to_array(user_id)
    ...
    Income.where(user_id: user_id).each do |income_data|
      ...
      income_hash.each do |k, v|
        if @incomes[k]
          @incomes[k].push({income_amount: v, type: IncomeType.find_by(id: income_data.income_type_id).name})
        else
          @incomes[k] = [{income_amount: v, type: IncomeType.find_by(id: income_data.income_type_id).name}]
        end
      end
    end  
    @incomes
  ...
  def compute_income_totals_for_a_date(income_amount, start_date, end_date, frequency_id, income_type_id)
    ...
    occurrences.each do |occurrence|
      ...
      # income_hash["Income Type"] = Income_type.find_by(id: income_type_id)
    end
    income_hash
  end
  ...
  def all_expense_hashes_to_array(user_id)
    ...
    Expense.where(user_id: user_id).each do |expense_data|
      ...
      expense_hash.each do |k, v|
        if @expenses[k]
          @expenses[k].push({expense_amount: v, type: ExpenseType.find_by(id: expense_data.expense_type_id).name})
        else
          @expenses[k] = [{expense_amount: v, type: ExpenseType.find_by(id: expense_data.expense_type_id).name}]
        end
      end
    end  
    p @expenses
